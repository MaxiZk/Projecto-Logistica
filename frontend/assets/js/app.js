...
const apiPut     = (p, body)  => body!=null ? apiFetch(p, { method: 'PUT',  body: JSON.stringify(body) })
                                            : apiFetch(p, { method: 'PUT' });
const apiDelete  = (p)        => apiFetch(p, { method: 'DELETE' });

/* ============== FACTURAS (API) ============== */
function apiListarFacturas(cliente) {
...
const apiAnularFacturaNC  = (id)      => apiPut(`/api/facturas/${encodeURIComponent(id)}/anular`);  // <-- SIN ?tipo
const apiObtenerFactura   = (id)      => apiGet(`/api/facturas/${encodeURIComponent(id)}`);
const apiEmitirNotaDebito = (id,p)    => apiPost(`/api/facturas/${encodeURIComponent(id)}/nota-debito`, p);
...
/* ===== Auth demo ===== */
const USERS = [
  { email:'gzuidwijk@zuidnet.com', password:'maxi1964!', role:'ADMIN' },
...
const Views = {
  home: () => `
    <div class="card p-4">
      ...
      <ul class="mb-2">
        <li>Terminales portuarias</li><li>Transportistas</li><li>Despachantes</li>
        <li>Depósitos para des/consolidados</li><li>Seguros</li>
      </ul>
    ...
    <div class="card p-3">
      ...
      <div class="table-responsive">
        <table class="table align-middle">
          <thead>
            <tr>
              <th>#</th><th>Cliente</th><th>Origen</th><th>Destino</th><th>Terminal</th>
              <th>Contenedor</th><th>Tipo</th><th>Tara (Kg)</th><th>Precinto</th>
              <th>Peso Bruto (Kg)</th><th>Chofer</th><th>DNI</th><th>Celular</th>
              <th>Tractor</th><th>Semi</th><th>Estado</th><th>Acciones</th>
            </tr>
          </thead>
  ...
  facturas: () => `
    <div class="card p-3">
      ...
      <div class="row g-2 mb-2">
        <div class="col-md-4"><input id="filterCliente" class="form-control" placeholder="Filtrar por cliente…"></div>
        <div class="col-md-2"><button id="btnBuscarFactura" class="btn btn-outline-secondary w-100">Buscar</button></div>
      </div>
      ...
      <div class="table-responsive">
        <table class="table align-middle">
          <thead>
            <tr>
              <th>#</th><th>Comprobante</th><th>Cliente</th><th>Fecha</th>
              <th>Detalle</th><th class="text-end">Neto</th><th class="text-end">IVA</th><th class="text-end">Total</th>
...
document.addEventListener('DOMContentLoaded', ()=>{
  document.querySelectorAll('#menu a[data-route]').forEach(a=>{
    a.addEventListener('click', (ev)=>{/* sólo hashchange */});
  });
  document.getElementById('logout')?.addEventListener('click', (e)=>{ e.preventDefault(); logout(); });
  setupCargasDelegation();   // delegación para Cargas
  updateAuthUI();
  render();
});
...
async function renderCargasTable(){
  ...
  tbody.innerHTML = (cargas||[]).map(c=>`
    <tr>
      <td>${safe(c.id)}</td><td>${safe(c.cliente)}</td><td>${safe(c.origen)}</td><td>${safe(c.destino)}</td>
      <td>${safe(c.terminalPortuaria)}</td><td>${safe(c.contenedor)}</td><td>${safe(c.tipo)}</td>
      <td>${safe(c.tara)}</td><td>${safe(c.precinto)}</td><td>${safe(c.pesoBruto)}</td>
      <td>${safe(c.chofer)}</td><td>${safe(c.dniChofer)}</td><td>${safe(c.celularChofer)}</td>
      <td>${safe(c.patenteTractor)}</td><td>${safe(c.patenteSemi)}</td>
...
function setupCargasDelegation(){
  ...
  document.getElementById('view').addEventListener('click', async (ev) => {
    ...
    if (btn.id === 'btnNuevaCarga') { showCargaFormAPI(null); return; }
    ...
    try {
      if (action === 'ver-carga') {
        const c = await apiObtenerCarga(id); showCargaViewFromObj(c);
      } else if (action === 'edit-carga') {
        if (getRole() !== 'ADMIN') { alert('Solo ADMIN.'); return; }
        const c = await apiObtenerCarga(id); showCargaFormAPI(c);
      } else if (action === 'del-carga') {
        if (getRole() !== 'ADMIN') { alert('Solo ADMIN.'); return; }
        if (!confirm(`¿Eliminar la orden ${id}?`)) return;
        await apiEliminarCarga(id); await renderCargasTable();
      }
    } catch (e) {
      console.error('Acción Cargas error:', e); alert('Operación de carga falló.');
    }
  });
}
...
function showCargaFormAPI(c){
  ...
  const init = c || {
    ...
    fecha:'', terminalPortuaria:'', contenedor:'', tipo:'',
  ...
  openModal({
    ...
    body: `
      <form id="formCarga" class="row g-3">
        <div class="col-md-2"><label class="form-label">ID Orden</label>
        ...
        <div class="col-md-4"><label class="form-label">Origen</label>
          <input class="form-control" name="origen" value="${init.origen||''}" required>
        </div>
        <div class="col-md-4"><label class="form-label">Destino</label>
          <input class="form-control" name="destino" value="${init.destino||''}" required>
        </div>
        <div class="col-md-4"><label class="form-label">Terminal Portuaria</label>
          <input class="form-control" name="terminalPortuaria" value="${init.terminalPortuaria||''}">
        </div>

        <div class="col-md-4"><label class="form-label">Contenedor</label>
        ...
        <div class="col-md-4"><label class="form-label">Precinto</label>
          <input class="form-control" name="precinto" value="${init.precinto||''}">
        </div>

        <div class="col-md-4"><label class="form-label">Tara (Kg)</label>
          <input type="number" step="0.01" class="form-control" name="tara" value="${init.tara||''}">
        </div>
        <div class="col-md-4"><label class="form-label">Peso bruto (Kg)</label>
          <input type="number" step="0.01" class="form-control" name="pesoBruto" value="${init.pesoBruto||''}">
        </div>

        <div class="col-md-4"><label class="form-label">Chofer</label>
          <input class="form-control" name="chofer" value="${init.chofer||''}">
        </div>
        <div class="col-md-4"><label class="form-label">DNI Chofer</label>
          <input class="form-control" name="dniChofer" value="${init.dniChofer||''}">
        </div>
        <div class="col-md-4"><label class="form-label">Celular Chofer</label>
          <input class="form-control" name="celularChofer" value="${init.celularChofer||''}">
        </div>

        <div class="col-md-4"><label class="form-label">Patente Tractor</label>
          <input class="form-control" name="patenteTractor" value="${init.patenteTractor||''}">
        </div>
        <div class="col-md-4"><label class="form-label">Patente Semi</label>
  ...
  document.getElementById('saveCarga').onclick = async ()=>{
    ...
    const payload = {
      id: fd.id, cliente: fd.cliente, origen: fd.origen, destino: fd.destino, estado: fd.estado, fecha: fd.fecha || null,
      terminalPortuaria: fd.terminalPortuaria || null, contenedor: fd.contenedor || null, tipo: fd.tipo || null,
      tara: fd.tara ? Number(fd.tara) : null, precinto: fd.precinto || null, pesoBruto: fd.pesoBruto ? Number(fd.pesoBruto) : null,
      chofer: fd.chofer || null, dniChofer: fd.dniChofer || null, celularChofer: fd.celularChofer || null,
      patenteTractor: fd.patenteTractor || null, patenteSemi: fd.patenteSemi || null
    };
    try{
      if(isEdit) await apiActualizarCarga(fd.id, payload); else await apiCrearCarga(payload);
      ensureModal().hide(); await renderCargasTable();
    }catch(e){ console.error(e); alert('No se pudo guardar la carga.'); }
  };
}

/* ============= LEGAJOS / TICKETS (UI) ============= */
async function renderLegajos(){
  const ul = document.getElementById('listLegajos'); if (!ul) return;
  try {
...
async function renderTickets(){
  const ul = document.getElementById('listTickets'); if (!ul) return;
  try {
...
/* ============= FACTURAS (UI) ============= */
const fNum   = v => (v == null || v === '' || isNaN(v)) ? 0 : Number(v);
const fMoney = v => (v == null || v === '' || isNaN(v)) ? '' : Number(v).toFixed(2);
const fSafe  = v => (v == null ? '' : v);
...
async function cargarFacturas() {
  try {
    ...
    tbody.innerHTML = (data || []).map(f => {
      ...
      let detalleVis = fSafe(f?.detalle);
      if (!detalleVis && Array.isArray(f?.detalles) && f.detalles.length > 0) {
        const it0 = f.detalles[0]; detalleVis = fSafe(it0?.descripcion);
      }

      let neto=f?.importeNetoGravado, iva=f?.importeIva, total=f?.importeTotal;
      if ((neto == null || total == null) && Array.isArray(f?.detalles)) {
        ...
        if (neto == null)  neto = sum; if (iva == null) iva = 0; if (total == null) total = neto + iva;
      }

      const acciones = (f?.estado !== 'ANULADA')
      ...
      return `
        <tr>
          <td>${fSafe(f?.id)}</td>
          <td><div>${fSafe(f?.tipo)} ${f?.codigoComprobante ? '(Cód. '+f.codigoComprobante+')' : ''}</div>
              <small class="text-muted">${comp}</small></td>
          <td>${fSafe(f?.cliente)}<br><small class="text-muted">${fSafe(f?.cuit) || '-'}</small></td>
          <td>${fSafe(f?.fechaEmision) || fSafe(f?.fecha) || '-'}</td>
          <td>${detalleVis || '-'}</td>
          <td class="text-end">${fmt(neto)}</td>
          <td class="text-end">${fmt(iva)}</td>
          <td class="text-end fw-semibold">${fmt(total)}</td>
          <td>${fSafe(f?.estado) || '-'}</td>
    ...
    console.error('GET /api/facturas falló:', e);
    alert('No se pudo cargar el listado de facturas.');
    return [];
  }
}

/* === Delegación de eventos para Facturas (única) === */
(function () {
  let FACTURAS_WIRED = false;
  window.setupFacturasDelegation = function () {
    if (FACTURAS_WIRED) return;
    const tbody = document.getElementById('tablaFacturas');
    if (!tbody) return;
    FACTURAS_WIRED = true;
    ...
    tbody.addEventListener('click', async (ev) => {
      ...
      try {
        if (action === 'anular-nc') {
          ...
          await apiAnularFacturaNC(id);        // PUT /api/facturas/{id}/anular
          await cargarFacturas();
        }
        if (action === 'ver') {
          const f = await apiObtenerFactura(id);
          renderFacturaDetalleModal(f);
        }
        if (action === 'nota-debito') {
          abrirModalNotaDebito(id);
        }
      } catch (e) {
        console.error('Acción Facturas error:', e);
...
function renderFacturaDetalleModal(f){
    ...
    itemsHtml = `
      <div class="table-responsive">
        <table class="table table-sm align-middle">
          <thead>
            <tr><th>#</th><th>Descripción</th><th class="text-end">U.M.</th>
                <th class="text-end">Cant.</th><th class="text-end">Precio</th><th class="text-end">Importe</th></tr>
          </thead>
          <tbody>${filas}</tbody>
        </table>
      </div>`;
  ...
  let neto=f.importeNetoGravado, iva=f.importeIva, otros=f.importeOtrosTributos, total=f.importeTotal;
  if ((neto==null || total==null) && Array.isArray(f.detalles)) {
    const sum = f.detalles.reduce((acc, it)=> acc + Number(it.cantidad ?? 0)*Number(it.precioUnitario ?? 0), 0);
    if (neto == null) neto = sum; if (iva == null) iva = 0; if (otros == null) otros = 0; if (total == null) total = neto + iva + otros;
  }
...
function abrirModalNotaDebito(id) {
  openModal({
    ...
    body: `
      <form id="formND" class="row g-3">
        <div class="col-md-6"><label class="form-label">Importe Neto</label>
          <input type="number" step="0.01" name="importeNeto" class="form-control" required>
        </div>
        <div class="col-md-6"><label class="form-label">Alícuota IVA (%)</label>
        ...
        <div class="col-md-6"><label class="form-label">Otros Tributos</label>
          <input type="number" step="0.01" name="otrosTributos" value="0" class="form-control">
        </div>
        <div class="col-md-12"><label class="form-label">Motivo</label>
  ...
  document.getElementById('btnEmitirND').onclick = async () => {
    ...
    try {
      await apiEmitirNotaDebito(id, payload);
      ensureModal().hide();
      await cargarFacturas();
    } catch (e) {
      console.error(e);
  ...
  btn.onclick = () => {
    openModal({
      ...
      <form id="formFactura" class="row g-3">
        <div class="col-md-2"><label class="form-label">Tipo</label>
          <select name="tipo" class="form-select"><option value="A">A (Cód.01)</option><option value="FCE A">FCE A (Cód.201)</option></select>
        </div>
        <div class="col-md-2"><label class="form-label">Cód.</label>
          <select name="codigoComprobante" class="form-select"><option value="01">01</option><option value="201">201</option></select>
        </div>
        <div class="col-md-2"><label class="form-label">Pto. Venta</label>
          <input type="number" name="puntoVenta" value="2" class="form-control">
        </div>
        <div class="col-md-3"><label class="form-label">Número</label>
        ...
        <div class="col-md-3"><label class="form-label">Cond. IVA</label>
          <input name="condicionIVACliente" value="IVA Responsable Inscripto" class="form-control">
        </div>
        <div class="col-md-12"><label class="form-label">Domicilio</label>
          <input name="domicilio" class="form-control">
        </div>

        <div class="col-md-4"><label class="form-label">Condición de venta</label>
          <input name="condicionVenta" value="Cuenta Corriente" class="form-control">
        </div>
        <div class="col-md-4"><label class="form-label">Vto. de pago</label>
          <input type="date" name="fechaVencimientoPago" class="form-control">
        </div>
        <div class="col-md-2"><label class="form-label">Período desde</label>
          <input type="date" name="periodoDesde" class="form-control">
        </div>
        <div class="col-md-2"><label class="form-label">Período hasta</label>
          <input type="date" name="periodoHasta" class="form-control">
        </div>

        <div class="col-md-12"><label class="form-label">Detalle</label>
          <textarea name="detalle" class="form-control" rows="3" placeholder="Transporte... EBKG..., B/L..., contenedores..."></textarea>
        </div>

        <div class="col-md-3"><label class="form-label">Cantidad</label>
          <input type="number" step="0.01" name="cantidad" class="form-control" required>
        </div>
        <div class="col-md-3"><label class="form-label">U. Medida</label>
          <input name="unidadMedida" value="unidades" class="form-control">
        </div>
        <div class="col-md-3"><label class="form-label">Precio Unit.</label>
          <input type="number" step="0.01" name="precioUnitario" class="form-control" required>
        </div>
        <div class="col-md-3"><label class="form-label">% Bonif.</label>
          <input type="number" step="0.01" name="bonificacionPct" value="0" class="form-control">
        </div>

        <div class="col-md-3"><label class="form-label">Alícuota IVA</label>
          <select name="alicuotaIvaPct" class="form-select"><option value="21">21%</option><option value="10.5">10.5%</option><option value="27">27%</option></select>
        </div>
        <div class="col-md-3"><label class="form-label">Otros Tributos</label>
        ...
        <div class="col-md-6"><div class="form-check mt.4">
          ...
          <label class="form-check-label" for="esFCE">FCE (mostrar CBU / Alias / Opc. Circulación)</label></div>
        </div>

        <div class="col-md-4"><label class="form-label">CBU Emisor</label>
          <input name="cbuEmisor" class="form-control" placeholder="0070...">
        </div>
        <div class="col-md-4"><label class="form-label">Alias CBU</label>
          <input name="aliasCbu" class="form-control" placeholder="ZUIDNETHSBC">
        </div>
        <div class="col-md-4"><label class="form-label">Opción Circulación</label>
          <input name="opcionCirculacion" class="form-control" value="Sistema de Circulación Abierta">
        </div>

        <div class="col-md-4"><label class="form-label">CAE</label>
          <input name="cae" class="form-control">
        </div>
        <div class="col-md-4"><label class="form-label">Vto. CAE</label>
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (!fd.cliente || !fd.cantidad || !fd.precioUnitario) {
        alert('Completá cliente, cantidad y precio unitario.'); return;
      }
      const payload = {
        tipo: fd.tipo || "A",
...
/* ===== Navegación dentro de Admin y Facturas ===== */
function wireListarFacturasButton() {
  ...
  if (btnBuscar) btnBuscar.onclick = () => cargarFacturas().then(setupFacturasDelegation);
}
...
function afterRender(route) {
  switch (route) {
    ...
    case 'facturas':
      wireListarFacturasButton();
      wireFacturaButton();
      cargarFacturas().then(setupFacturasDelegation);   // <-- importante
      break;
    default:
      break;
  }
